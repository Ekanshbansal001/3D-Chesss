...
function renderChoiceGrid(container, options, initialKey, onSelect) {
  container.innerHTML = "";
  options.forEach(opt => {
    const btn = document.createElement("div");
    btn.className = "tc-btn" + (opt.key === initialKey ? " active" : "");
...
renderChoiceGrid($("#aiColorGrid"), [
  { key: "white", label: "White" },
  { key: "black", label: "Black" },
  { key: "random", label: "Random" }
], aiColor, (k) => aiColor = k);

renderChoiceGrid($("#aiDifficultyGrid"), [
  { key: "easy", label: "Easy" },
  { key: "medium", label: "Medium" },
  { key: "hard", label: "Hard" },
  { key: "expert", label: "Expert" }
], aiDifficulty, (k) => aiDifficulty = k);

$("#playAiBtn").addEventListener("click", () => openModal("#aiModal"));
...
$("#confirmAiStart").addEventListener("click", () => {
  Audio_.notify();
  window.location.href = `ai.html?color=${aiColor}&difficulty=${aiDifficulty}`;
});
