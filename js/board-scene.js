...
export const BOARD_THEMES = {
  obsidian: { light: 0x4c4a54, dark: 0x18161c, border: 0x1e1c24, frame: 0x28252f },
  royalwood: { light: 0xc99b66, dark: 0x6b4527, border: 0x332013, frame: 0x4a2f1e },
  marble: { light: 0xede8dc, dark: 0xc2b8a9, border: 0x968d7d, frame: 0x7c7568 }
};
...
function buildPawn(mat) {
  ...
  const stem = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.13, 0.28, 14), mat); stem.position.y = 0.24; g.add(stem);
  const head = new THREE.Mesh(new THREE.SphereGeometry(0.13, 14, 10), mat); head.position.y = 0.44; g.add(head);
...
function buildRook(mat) {
  ...
  const body = new THREE.Mesh(new THREE.CylinderGeometry(0.17, 0.2, 0.42, 14), mat); body.position.y = 0.31; g.add(body);
  const top = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.19, 0.08, 14), mat); top.position.y = 0.56; g.add(top);
  for (let i = 0; i < 6; i++) {
...
function buildKnight(mat) {
  ...
  const neck = new THREE.Mesh(new THREE.CylinderGeometry(0.13, 0.18, 0.36, 12), mat); neck.position.y = 0.28; g.add(neck);
  ...
  const ear = new THREE.Mesh(new THREE.ConeGeometry(0.05, 0.14, 7), mat);
...
function buildBishop(mat) {
  ...
  const body = new THREE.Mesh(new THREE.ConeGeometry(0.17, 0.5, 14, 1, false), mat); body.position.y = 0.4; g.add(body);
  const collar = new THREE.Mesh(new THREE.TorusGeometry(0.12, 0.025, 6, 14), mat); collar.position.y = 0.58; collar.rotation.x = Math.PI / 2; g.add(collar);
  const tip = new THREE.Mesh(new THREE.SphereGeometry(0.06, 10, 8), mat); tip.position.y = 0.72; g.add(tip);
...
function buildQueen(mat) {
  ...
  const body = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.19, 0.46, 16), mat); body.position.y = 0.33; g.add(body);
  const collar = new THREE.Mesh(new THREE.TorusGeometry(0.15, 0.03, 6, 16), mat); collar.position.y = 0.58; collar.rotation.x = Math.PI / 2; g.add(collar);
  const crown = new THREE.Mesh(new THREE.SphereGeometry(0.13, 14, 10), mat); crown.position.y = 0.7; g.add(crown);
  for (let i = 0; i < 8; i++) {
    const ang = (i / 8) * Math.PI * 2;
    const spike = new THREE.Mesh(new THREE.ConeGeometry(0.025, 0.09, 6), mat);
...
function buildKing(mat) {
  ...
  const body = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.2, 0.52, 16), mat); body.position.y = 0.36; g.add(body);
  const collar = new THREE.Mesh(new THREE.TorusGeometry(0.16, 0.03, 6, 16), mat); collar.position.y = 0.64; collar.rotation.x = Math.PI / 2; g.add(collar);
  const headBall = new THREE.Mesh(new THREE.SphereGeometry(0.1, 12, 8), mat); headBall.position.y = 0.76; g.add(headBall);
...
export class BoardScene {
  ...
  _initScene() {
    ...
    renderer.setClearColor(0x141019, 1);
    // Shadows only at medium/high — the single biggest lag source on phones
    // is real-time shadow casting from ~30 small meshes at once.
    ...
    // Lighter, shorter fog so the board doesn't feel like it's sitting in a void.
    this.scene.fog = new THREE.Fog(0x1c1722, 9, 20);
    ...
    // Key light — brighter and warmer so pieces read clearly against the board.
    const key = new THREE.DirectionalLight(0xfff6e4, 1.65);
    ...
    // Soft second key from the opposite side so no face of a piece is ever fully dark.
    const keyFill = new THREE.DirectionalLight(0xffe9c4, 0.55);
    keyFill.position.set(-3, 5, -2);
    this.scene.add(keyFill);

    const fill = new THREE.DirectionalLight(0xc9a45c, 0.4);
    fill.position.set(-5, 3, -4);
    this.scene.add(fill);

    const rim = new THREE.PointLight(0xe8c77c, 0.6, 14);
    rim.position.set(0, 3, -4);
    this.scene.add(rim);

    // Hemisphere light lifts overall brightness with a natural sky/ground
    // gradient instead of flattening everything with more ambient light.
    const hemi = new THREE.HemisphereLight(0x8a94b8, 0x2a2018, 0.65);
    this.scene.add(hemi);

    const ambient = new THREE.AmbientLight(0x9a8f7a, 0.85);
  ...
  _applyBoardTheme() {
    ...
    this._lightMat = new THREE.MeshStandardMaterial({ color: t.light, metalness: 0.1, roughness: 0.42 });
    this._darkMat = new THREE.MeshStandardMaterial({ color: t.dark, metalness: 0.1, roughness: 0.4 });
  }
  ...
  setQuality(q) {
    ...
    this.renderer.setPixelRatio(this._pixelRatioForQuality(q));
    // Re-apply shadow casting on existing pieces to match the new tier.
    this.pieces.forEach(group => {
      group.traverse(obj => { if (obj.isMesh) obj.castShadow = q === "high"; });
    });
  }

  _pixelRatioForQuality(q) {
    if (q === "high") return Math.min(window.devicePixelRatio, 1.75);
    if (q === "medium") return Math.min(window.devicePixelRatio, 1.25);
    return 1;
  }
  ...
  _placePiece(sq, type, color) {
    ...
    group.traverse(obj => { if (obj.isMesh) { obj.castShadow = this.quality === "high"; obj.receiveShadow = true; } });
    ...
    return new Promise(resolve => {
      ...
      const step = (now) => {
        ...
        if (promotion) {
          ...
          newGroup.traverse(obj => { if (obj.isMesh) { obj.castShadow = this.quality === "high"; obj.receiveShadow = true; } });
  ...
  _resize() {
    ...
    this.renderer.setPixelRatio(this._pixelRatioForQuality(this.quality));
