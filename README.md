...
```
/
  index.html            Landing page (create/join/play online/vs computer)
  game.html              Multiplayer table — 3D board, clocks, move history
  ai.html                 Offline vs-computer table
  css/
    main.css             Design tokens + landing page styles
    game.css              Game-screen layout and states (shared by game.html + ai.html)
  js/
    firebase-config.js    ⚠️ Edit this — your Firebase project keys (multiplayer only)
    ...
    game.js                        Multiplayer table controller
    ai-engine.js                   Stockfish wrapper (runs in a Web Worker)
    simple-bot.js                   Fallback bot (used only if Stockfish can't load)
    ai-controller.js                 Difficulty presets + engine/fallback switch
    ai-game.js                        Offline vs-computer table controller
  assets/
    favicon.svg
...
| **Play vs Computer** (offline, no Firebase needed) | ✅ — real Stockfish engine via WebAssembly/Web Worker, 4 difficulty levels, choose your color |
